...
# API_URL = "https://gkafoif3y2.execute-api.us-east-1.amazonaws.com/dev"
API_URL = "https://r77zhyckrg.execute-api.us-east-1.amazonaws.com/"
REGION = "us-east-1"
...
<option value="{{ inst['InstanceId'] }}">{{ inst['InstanceId'] }} - {{ inst['State'] }} - {{ inst['PublicIpAddress'] or 'No Public IP' }}</option>
{% endfor %}
</select>
</div>
</form>
...
<div class="alert {{ 'alert-success' if ('created' in lower or 'success' in lower or 'started' in lower or 'stopped' in lower or 'terminated' in lower) else ('alert-warning' if 'please' in lower else ('alert-danger' if 'error' in lower else 'alert-info')) }} mt-3">
...
def ec2_control():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if body_dict:
                message = (body_dict.get("message") or body_dict.get("Message") or body_dict.get("msg") or "")
